...
| RGB SLEEP | Real 10-minute idle → RGB off; key wake; OS USB suspend; powered hub + PC off (~300 ms SOF stale); charger-only never enumerated stays lit; usevia.app SYSTEM → SLEEP GET/SET/SAVE and reboot; EEPROM CLEAN → 10 min. |
