...
- 개발 단계에서만 필요했던 계측 경로이므로, `_DEF_ENABLE_MATRIX_TIMING_PROBE`와 `_DEF_ENABLE_USB_TIMING_LOG`(가칭)를 `src/hw/hw_def.h`에 정의해 매크로 조건부 컴파일로 묶는 방안을 권장합니다. 기본값은 `0`으로 두고 개발 환경에서만 `1`로 재정의하면, 펌웨어 릴리스 빌드에서 계측 관련 코드가 완전히 제외되어 타이머 접근과 로그 버퍼 연산이 제거됩니다.
- 위 매크로를 적용하면 `matrix_scan()`과 HID 진단 루틴의 계측 코드가 동일한 토글 정책을 따르게 되어, 디버그용 UF2/빌드에서만 계측 데이터를 확보하고 일반 사용자 빌드에서는 오버헤드가 0이 됩니다. 매크로 정의 위치를 `hw_def.h`로 통일하면 CLI/빌드 스크립트가 동일한 플래그를 참고할 수 있어 운용 복잡도를 줄일 수 있습니다.
...
| B | `matrix_scan()`의 `key_scan_time`/`usbHidSetTimeLog` 계측을 `is_info_enable` 등 런타임 플래그 + `_DEF_ENABLE_MATRIX_TIMING_PROBE`/`_DEF_ENABLE_USB_TIMING_LOG` 컴파일 가드로 이중 제어 | 평시 `micros()` 호출 감소로 스캔 루프 CPU 여유 확보, 릴리스 빌드에서 계측 코드 완전 제거 | 실시간 로깅을 즉시 활성화할 때 직전 스캔 값이 비어 있을 수 있음, 매크로 관리 복잡도 증가 | 조건부 적용 검토 (디버그 요구사항 확인 후 결정) |
