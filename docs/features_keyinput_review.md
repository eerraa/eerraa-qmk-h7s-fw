# 키 입력 경로 리팩토링/최적화 리뷰 (기준: V251009R4)
...
- 릴리스 프로파일에서는 계측 경로가 사용되지 않도록 `_DEF_ENABLE_MATRIX_TIMING_PROBE`와 `_DEF_ENABLE_USB_TIMING_LOG`를 기본값 `0`으로 두고, 디버그 빌드에서만 `1`로 오버라이드하여 컴파일 가드를 통과시키는 형태로 정리했습니다. 이렇게 하면 릴리스 바이너리에서는 `key_scan_time` 산출과 `usbHidSetTimeLog()` 호출이 제거되고, 디버그용 빌드에서만 동일한 코드를 재사용할 수 있습니다. 매크로 정의 위치는 `hw_def.h`로 고정해 빌드 스크립트와 CLI가 동일한 토글을 참조합니다.
- 매크로가 `0`인 릴리스 빌드에서 디버그 CLI를 즉시 켜더라도 직전 측정값이 비어 있는 상황을 대비해, CLI 진입 시 첫 샘플이 적재되기 전까지는 스캔 시간/USB 타임로그 출력이 공란일 수 있음을 명시해 운용 혼선을 방지합니다.
...
| B | `matrix_scan()`의 `key_scan_time`/`usbHidSetTimeLog` 계측을 `is_info_enable` 등 런타임 플래그 + `_DEF_ENABLE_MATRIX_TIMING_PROBE`/`_DEF_ENABLE_USB_TIMING_LOG` 컴파일 가드로 이중 제어 | 평시 `micros()` 호출 감소로 스캔 루프 CPU 여유 확보, 릴리스 빌드에서 계측 코드 완전 제거 | 실시간 로깅을 즉시 활성화할 때 직전 스캔 값이 비어 있을 수 있음, 매크로 관리 복잡도 증가 | 적용 (V251009R4, 릴리스 빌드 기본 비활성) |
...
- 제안 A, C는 영향 범위가 크므로 후속 테스트 계획 수립 후 적용 여부를 결정하는 것이 바람직합니다. 제안 B는 릴리스 프로파일에서 계측 경로를 제거하는 형태로 확정 적용했으며, 디버그 빌드에서만 매크로 토글로 활성화합니다.
