...
- 행에 다수의 키 변화가 있는 우에도 모든 비트를 순회하므로 기존 이벤트 순서와 기능 동일.
...
### 4.1 입력 흐름 추적 및 병목 파악
- **경로**: `keyboard_task()` → `matrix_task()` → `has_ghost_in_row()` → `get_real_keys()`.
- **현상**: 행 단위 변경이 감지되면 `has_ghost_in_row()`가 호출되고, 여기서 `get_real_keys()`가 모든 열을 순회하며 키맵 상의 유효 키만 남겨둠.
- **문제점**: 실제 사용에서는 동일 행에서 동시에 눌리는 키 수가 0~1개인 경우가 대부분인데, 이 경우에도 매번 `get_real_keys()`가 호출되어 키맵 조회와 비트 연산이 반복됨.

### 4.2 개선안과 오리지널 코드 비교
| 구분 | 기존 구현 | 개선안(V250924R8) |
|------|-----------|-------------------|
| 초기 분기 | 행 데이터와 관계없이 즉시 `get_real_keys()` 수행 | 물리 행 데이터의 비트 수가 0/1이면 조기 반환 |
| 키맵 조회 | 모든 호출에서 열 전체를 순회 | 2키 이상이 눌린 경우에만 실행 |
| 성능 영향 | 단일 키 입력에서도 키맵 조회 비용 발생 | 대부분의 입력(0/1키)에서 함수 호출 한 번으로 종료 |

### 4.3 오리지널 의도 분석
- 키맵상 정의된 키만 대상으로 고스트 여부를 판단해, 미할당 위치(KC_NO)에서 발생하는 잘못된 고스트 판정을 방지하려는 구조.
- 행에 2개 이상의 유효 키가 존재할 때만 고스트 여부를 확인하고, 발견 시 `matrix_previous` 갱신을 보류해 안전한 해제 이벤트를 확보하려는 목적.

### 4.4 부작용 검토
- **고스트 판정 누락 여부**: 고스트는 최소 2개 이상의 스위치가 동시에 눌릴 때만 발생하므로, 물리 행 데이터에서 비트 수가 0/1이면 고스트가 존재할 수 없음. 조기 종료가 원래 의도와 충돌하지 않음.
- **키맵 필터 영향**: 2개 이상의 비트가 있는 경우에는 기존과 동일하게 `get_real_keys()`가 호출되어, KC_NO 필터링과 타 행 비교 로직이 유지됨.
- **호환성**: `matrix_task()`의 고스트 처리 플래그(`ghost_pending`) 및 후속 이벤트 흐름은 전혀 변경되지 않음.

### 4.5 수정/유지 판단
- **수정 필요**: 단일 키 입력 시 불필요하게 반복되는 키맵 순회 비용을 제거하면 `matrix_task()`가 다루는 대부분의 스캔에서 즉시 반환되어 CPU 여유가 확보됨.
- **유지 항목**: 2키 이상에서의 고스트 판정, `switch_events()` 호출 순서, 디바운스/이벤트 연계 구조는 그대로 유지.
