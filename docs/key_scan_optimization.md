...
## 10. V251001R1: tick 이벤트 타임스탬프 공유로 `timer_read()` 중복 제거

### 10.1 입력 흐름 재검토
- **추적 경로**: `keyboard_task()` → `matrix_task()` → `generate_tick_event()` → `action_exec()`.
- **현황**: `generate_tick_event()`가 tick 간격을 확인하기 위해 `timer_read()`를 호출한 뒤, `action_exec(MAKE_TICK_EVENT)` 경로에서 `MAKE_EVENT` 매크로가 다시 `timer_read()`를 실행한다.

### 10.2 개선안과 오리지널 코드 비교
| 구분 | 기존 구현 | 개선안(V251001R1) |
|------|-----------|------------------|
| 타이머 판정 | `generate_tick_event()`에서 `timer_read()` | 동일 |
| 이벤트 생성 | `MAKE_TICK_EVENT` 매크로가 두 번째 `timer_read()` 실행 | tick 이벤트 구조체를 직접 구성해 첫 번째 측정값을 재사용 |
| 호출 수 | tick 발생 시 `timer_read()` 2회 | tick 발생 시 `timer_read()` 1회 |

### 10.3 오리지널 의도 분석
- tick 이벤트는 내부 상태 머신(탭-홀드 타임아웃, 오토시프트 등)을 구동하기 위해 1kHz 최대 빈도로 발생해야 한다.
- 이벤트마다 타임스탬프를 부여해 `timer_elapsed()` 비교가 가능하도록 하려는 목적이므로, `MAKE_TICK_EVENT`를 통한 즉시 생성이 채택돼 있었다.

### 10.4 부작용 검토
- **타임스탬프 정확도**: 기존에도 두 호출이 거의 동일한 값을 반환했으므로, 첫 번째 측정값을 재사용해도 1ms 단위 타임라인이 유지된다.
- **매크로 호환성**: tick 이벤트만 직접 생성하고 나머지 이벤트는 기존 매크로를 유지하므로, 다른 호출부와 충돌 가능성이 없다.
- **API 안정성**: `keyevent_t` 필드를 명시적으로 채워도 구조체 초기화 순서가 매크로와 동일해 역호환 문제가 없다.

### 10.5 수정/유지 판단
- **수정 필요**: tick 이벤트 생성 시 중복된 `timer_read()` 호출을 제거해 타이머 레지스터 접근 비용을 줄인다.
- **유지 항목**: tick 간격 판정 로직, `action_exec()` 호출 시점, `last_tick` 갱신 방식은 기존 구현을 그대로 따른다.

### 10.6 재검토 결과
- `generate_tick_event()`가 반환하는 타임스탬프와 이벤트 타임스탬프가 일치해 디버그 로깅이나 `timer_elapsed()` 기반 비교에서 더 일관된 결과를 확인했다.
- tick 이벤트 경로 외에는 매크로 사용 방식을 변경하지 않아 추가 보완 사항이 발생하지 않았다.

## 11. V251001R2: 고스트 재검사 시 `timer_read()` 낭비 제거

### 11.1 입력 흐름 재검토
- **추적 경로**: `keyboard_task()` → `matrix_task()` → `should_process_keypress()` → `action_exec()`.
- **현황**: 행 변화가 감지되면 고스트 검사를 통과하기 전에 `should_process_keypress()`와 `timer_read()`를 호출해 이벤트 템플릿을 만든다.
- **비용 요인**: 고스트로 판정돼 이벤트가 발생하지 않더라도 `timer_read()`가 매 스캔마다 실행되어 타이머 레지스터 접근이 반복된다.

### 11.2 개선안과 오리지널 코드 비교
| 구분 | 기존 구현 | 개선안(V251001R2) |
|------|-----------|------------------|
| 이벤트 준비 시점 | 행 루프 진입 직후 고정 호출 | 고스트 검사를 통과한 첫 행에서만 지연 호출 |
| 타임스탬프 측정 | 모든 행 변화에서 `timer_read()` 실행 | 실제 키 이벤트가 존재할 때만 `timer_read()` 실행 |
| 함수 호출 수 | 고스트 상태에서도 `should_process_keypress()` 호출 | 고스트만 있는 스캔에서는 호출 생략 |

### 11.3 오리지널 의도 분석
- 분할 키보드 등에서 슬레이브가 이벤트를 처리하지 않도록 `should_process_keypress()`로 마스터 여부를 판별한다.
- 모든 키 이벤트가 동일 스캔 타임스탬프를 공유하도록 스캔 시점에서 `timer_read()`를 수행한다.
- 고스트가 감지되면 `action_exec()` 호출을 건너뛰어 잘못된 이벤트를 차단한다.

### 11.4 부작용 검토
- **마스터 판별**: 실제로 키 이벤트가 생성되는 시점에만 `should_process_keypress()`를 호출하므로 마스터/슬레이브 판별 흐름은 유지된다.
- **타임스탬프 일관성**: 키 이벤트가 없는 스캔에서는 타임스탬프가 필요 없으므로 생략해도 이후 상태 머신(tap-hold, 콤보 등)에 영향이 없다.
- **고스트 처리**: 고스트 행은 검사를 통과하지 못하면 여전히 건너뛰므로 지연 초기화가 이벤트 누락을 유발하지 않는다.

### 11.5 수정/유지 판단
- **수정 필요**: 실제 키 이벤트가 확정된 이후에만 이벤트 템플릿을 초기화해 불필요한 `timer_read()` 호출과 함수 호출을 제거한다.
- **유지 항목**: 고스트 검출 로직, `switch_events()` 호출, 스캔 단위 타임스탬프 공유 방식은 기존과 동일하게 유지한다.

### 11.6 재검토 결과
- 고스트 상태가 연속될 때 `should_process_keypress()`와 `timer_read()` 호출이 생략되어도 고스트 해제 시점에 즉시 이벤트가 생성되는 것을 확인했다.
- 슬레이브 측에서는 여전히 첫 키 이벤트가 등장할 때만 `should_process_keypress()`가 실행되어 기존 분기 구조와 충돌하지 않는다.

## 12. V251001R3: 키 이벤트·활동 타임스탬프 공유로 `sync_timer_read32()` 중복 제거

### 12.1 입력 흐름 재점검
- **추적 경로**: `keyboard_task()` → `matrix_task()` → `action_exec()` 및 `last_matrix_activity_trigger()`.
- **현상**: `matrix_task()`가 첫 키 변화를 감지하면 `timer_read()`로 이벤트용 16비트 타임스탬프를 확보하고, 직후 `keyboard_task()`는 `last_matrix_activity_trigger()`를 호출해 `sync_timer_read32()`(= `timer_read32()`)를 다시 실행한다.
- **문제점**: 동일 스캔에서 16비트/32비트 타이머를 각각 읽어 두 번의 `millis()` 호출이 발생해, 8000Hz 루프에서 불필요한 타이머 접근 비용이 중첩된다.

### 12.2 개선안과 오리지널 코드 비교
| 구분 | 기존 구현 | 개선안(V251001R3) |
|------|-----------|-------------------|
| 키 이벤트 타임스탬프 | `timer_read()`로 개별 획득 | 첫 행 변화 시 `sync_timer_read32()`를 1회 읽고 하위 16비트를 공유 |
| 활동 타임스탬프 | `last_matrix_activity_trigger()`에서 `sync_timer_read32()` 재호출 | `matrix_task()`가 기록한 32비트 값을 재사용, 미기록 시에만 재호출 |
| 고스트 처리 | 고스트 행에서도 타이머 호출이 발생 | 고스트만 존재할 때도 최초 변화 시점의 타임스탬프를 공유 |

### 12.3 오리지널 의도 분석
- `last_matrix_activity_trigger()`는 32비트 정밀도를 유지해 USB 절전/입력 감지 로직을 구동한다.
- 키 이벤트는 16비트(`timer_read`) 기반이지만, 스캔마다 동일 타임스탬프를 공유해 탭-홀드/콤보 동작의 일관성을 확보한다.
- 두 함수 모두 동일한 스캔 시각을 사용하지만 서로 독립적으로 타이머를 읽어 중복 비용이 누적됐다.

### 12.4 부작용 검토
- **타임스탬프 일관성**: 32비트 값을 하위 16비트로 절단해도 `timer_read()`와 동일한 `millis()` 소스를 사용하므로 오버플로우 타이밍과 분해능이 유지된다.
- **예외 처리**: 실제 키 변화 없이 `matrix_task()`가 true를 반환하는 드문 상황(예: `ghost_pending` 유지)에서도 `last_matrix_activity_trigger()`가 이전처럼 `sync_timer_read32()`를 호출하도록 폴백 경로를 남겼다.
- **동시성**: `pending_matrix_activity_time`은 `matrix_task()` → `keyboard_task()` 단일 쓰레드 흐름에서만 갱신되므로 추가 잠금이 필요 없다.

### 12.5 수정/유지 판단
- **수정 필요**: 최초 행 변화 시점에 32비트 타임스탬프를 1회만 확보하고, 이를 키 이벤트와 활동 타임스탬프가 공유하도록 `matrix_task()`와 `last_matrix_activity_trigger()`를 보완한다.
- **유지 항목**: 고스트 판정, `should_process_keypress()` 호출 위치, `switch_events()` 연계 구조는 기존과 동일하게 유지해 기능적 호환성을 보장한다.

### 12.6 재검토 결과
- `matrix_task()`에서 고스트로 인해 이벤트가 생성되지 않아도 활동 타이머가 갱신돼 슬립 타이머가 정상적으로 리셋되는 것을 확인했다.
- `pending_matrix_activity_time`이 비어 있을 경우 기존 경로가 그대로 실행돼, 예상치 못한 타이머 누락 없이 안정적으로 동작함을 검증했다.

## 13. V251001R4: 고스트 판정용 행 캐시로 키맵 필터 재계산 최소화

### 13.1 입력 흐름 재점검
- **추적 경로**: `keyboard_task()` → `matrix_task()` → `has_ghost_in_row()` → `get_real_keys()` → `keycode_at_keymap_location()`.
- **현상**: 동일 스캔 내에서 여러 행이 동시에 변경되면, 각 행의 고스트 판정이 서로를 참조하면서 `get_real_keys()`가 반복 호출된다.
- **비용 요인**: `get_real_keys()`는 행별 키맵 마스크 캐시를 갱신하고, VIA/Vial 환경에서는 EEPROM을 읽는 `dynamic_keymap_get_keycode()`까지 이어져 누적 지연이 발생한다.

### 13.2 개선안과 오리지널 코드 비교
| 구분 | 기존 구현 | 개선안(V251001R4) |
|------|-----------|-------------------|
| 행 필터링 | `has_ghost_in_row()` 진입 시마다 `get_real_keys()` 호출 | 스캔마다 행 필터 결과를 캐시에 저장해 재사용 |
| 캐시 범위 | 없음 | `matrix_task()` 1회 실행 동안만 유효한 세대(epoch)를 부여 |
| 오버플로우 대응 | 고려 사항 없음 | epoch가 32비트 한계를 넘으면 캐시를 강제 무효화 |

### 13.3 오리지널 의도 분석
- 키맵 필터는 물리적으로 존재하지만 `KC_NO`로 비워진 위치를 제외해, 고스트 판정이 실제 사용 가능한 키만 대상으로 동작하도록 설계돼 있다.
- `has_ghost_in_row()`는 다른 행과의 중복 열을 비교해야 하므로, 정확한 필터 결과를 공유하는 것이 필수다.
- 기존 구현은 단순 재호출 구조로 신뢰성을 확보했지만, 동일 스캔 안에서 중복 계산이 빈번했다.

### 13.4 부작용 검토
- **동적 키맵 호환성**: 행 캐시는 단일 스캔(epoch) 안에서만 재사용되며, 동적 키맵이 변경되면 dirty 플래그가 이미 행 마스크를 무효화하므로 새 스캔에서 즉시 최신 상태가 반영된다.
- **메모리 사용량**: `MATRIX_ROWS` 크기의 `matrix_row_t` 배열과 epoch 배열을 추가하지만, STM32H7S 메모리 여유와 비교하면 미미하며 스캔 지연 감소 효과가 더 크다.
- **오버플로우 처리**: 32비트 epoch가 순환할 때 모든 캐시의 epoch를 0으로 초기화해, 오래된 데이터가 새 스캔에서 재사용되는 상황을 차단했다.

### 13.5 수정/유지 판단
- **수정 필요**: 동일 스캔에서 반복 호출되는 `get_real_keys()`를 행 캐시로 대체해, 키맵 필터 연산과 EEPROM 접근을 줄인다.
- **유지 항목**: 고스트 판정 조건, `switch_events()` 호출 구조, `ghost_pending` 처리 방식은 기존과 동일하게 유지해 동작 호환성을 보장한다.

### 13.6 재검토 결과
- 행 캐시가 동일 스캔 내부에서만 재사용되어 키 입력 시퀀스나 고스트 판정 결과가 기존과 동일하게 유지됨을 확인했다.
- epoch 오버플로우 시 캐시를 초기화하는 경로가 정상 동작해, 장시간 동작 후에도 잘못된 키맵 데이터가 사용되지 않았다.
