...
## 2026-08-24 — VIA 메뉴 Anti-Ghosting을 KKUK으로 개명 (V260824R2 동봉, 펌웨어 코드 변경 없음)
...
`eerraa-qmk-h7s-fw-handover/hardware-validation-V260824R1.md`.

### 가장 중요한 사실 — HID 지연의 절대값은 열거마다 달라진다

같은 펌웨어·같은 모드인데 회차 간 min/avg가 크게 이동했다.

| 모드 | 1차 min/avg | 2차 min/avg |
| --- | --- | --- |
| FS 1K | 166 / 231 µs | **512 / 558 µs** |
| HS 2K | 150 / 224 µs | **326 / 370 µs** |
| HS 4K | 8 / 232 µs | **140 / 184 µs** |
| HS 8K | 6 / 85 µs | 7 / 85 µs |

D-1 때문이 아니다. D-1은 큐 대기를 줄이므로 지연을 올릴 수 없고, 실제로 span은 줄었다.

**원인은 열거 시점에 고정되는 임의 위상이다.** 근거:
- 같은 부팅 안의 두 런은 `min`이 동일하다 — 1차 8K: 6/6, 2차 8K: 7/7
- 다른 부팅이면 달라진다 — 1차 8K 다른 부팅: min 5, avg 61
- 모드별 `min`이 회차마다 제각기 이동한다(166→512, 150→326, 8→140)

`min`이 곧 위상이다. 리포트가 디바운스 1 ms 틱 경계에서 방출되고(`DEBOUNCE 5`,
`timer_read_fast`, 메인 루프 267 kHz라 경계에 4 µs 이내로 붙는다) 그 틱과 호스트 USB
프레임의 위상차가 부팅마다 무작위로 정해지기 때문이다. 8K만 안정적으로 보이는 것은
군집 폭(~110 µs)이 간격(125 µs)을 거의 채워 위상이 상쇄되기 때문이다.

**따라서 min/avg/max 절대값은 회차 간·모드 간 비교에 쓸 수 없다.** 2026-08-24 1차
분석에서 "정규화 대신 절대 µs로 비교하라"고 결론냈던 것은 불충분했다. 비교 가능한 것은
위상 독립 지표뿐이다: **span(max−min)**, queue depth peak, report drops,
`>2× interval` 건수, main-loop gap. 앱 비교표에 `Spread`(span/interval)와 `Queue` 열을
추가하고 Avg/Max에 "연결마다 재추첨되는 고정 오프셋이 포함된다"고 명시했다.

### D-1 수정 효과 (위상 독립 지표로 측정)

| 모드 | span 1차 → 2차 | queue peak | >2×E 건수 |
| --- | --- | --- | --- |
| FS 1K | 1.07E → 1.05E | 1 → 1 | 0 → 0 |
| HS 2K | 2.03E → **1.10E** | **2 → 1** | **5 → 0** |
| HS 4K | 2.65E → **1.17E** | **2 → 1** | **3 → 0** |
| HS 8K | 1.81~2.65E → 1.83E | 2 → 1 | 3 → 0 |

재시도 꼬리가 2K·4K에서 절반 이하로 줄고 `>2× interval` 버킷이 전 세션 0이 됐다.
타이핑+마우스키 동시 세션은 947 samples(최다)에 drop 0·queue peak 1이고 사용자도 씹힘
없음으로 답해 **keyboard/EXK 동시 전송이 정상 동작**함을 확인했다.
단, 1·2차는 열거·타이핑·시각이 달라 통제된 A/B가 아니다. 2K·4K에서 일관된 점이 근거다.

### 회귀 확인 (1차에서 비어 있던 항목)

- **BootMode 영속성 통과** — 3번 세션을 USB 완전 분리 후 실행했고 덤프에
  `pollingMode:2 / speed:High`로 기록됐다. 재연결 후에도 HS 4K가 유지된다.
- EEPROM Reset 후 FS 1K 복귀 통과. 진단 세션 중 키맵·조명 변경 정상 반영.
  키 씹힘/붙음 없음. 화면 이상 표시 없음.
- 9개 세션 전부 report drop 0, 1 ms 초과 stall 0, snapshot sequence 연속.

### 부팅 카운터 읽는 법 — 두 번 오해를 샀으므로 기록한다

"절전해도 `Suspends`가 1에서 안 변한다"는 관측이 나왔으나 **계측은 정상이다.** 같은
덤프의 시간축이 증명한다.

```
19:54:21  중단 런     boot resets/cfg/suspends/speedChanges = 1/1/1/0
20:03:20  다음 세션   boot resets/cfg/suspends/speedChanges = 8/4/7/4
```

1차의 케이블 분리·절전 테스트 구간에서 suspends가 1 → 7로 올라갔다. 빌드된 바이너리에서도
`HAL_PCD_SuspendCallback` → `usbDiagnosticsOnUsbSuspend` 호출을 디스어셈블리로 확인했다
(호출자 2곳: USBSUSP, LPM). `USE_HAL_PCD_REGISTER_CALLBACKS=0`이라 `usbd_conf.c`의 강한
심볼이 링크되고 `USBSUSPM`도 언마스크되어 있다.

변화를 못 보는 이유는 둘 다 UI 쪽이다.

1. **`Since firmware boot`는 실시간 값이 아니라 스냅샷 값이다.** 세션이 새 스냅샷을 만들
   때만 갱신된다. 이벤트 전후로 읽어도 세션을 새로 돌리지 않으면 같은 옛 스냅샷을 두 번
   읽게 된다.
2. **폴링 모드 Apply는 MCU를 재부팅시켜 카운터를 0으로 되돌린다.** 덤프에서 모드 변경 뒤
   실행한 런은 예외 없이 1/1/1/0이다. 누적이 보인 20:03/20:07 구간만 앞에 모드 변경이
   없었다.

따라서 **어떤 이벤트가 계수됐는지 보려면 이벤트를 일으킨 뒤 짧은 세션을 새로 돌려야 한다.**
결과 화면의 해당 패널에 이 문구를 넣었다.

### 미해결

7번 세션(세션 중 절전)이 덤프에 기록되지 않았다. 저장된 V260824R1 런은 6개이고 중단 런은
케이블 분리 1건뿐이다. 펌웨어는 완료 세션을 CLEAR/다음 START 전까지 RAM에 유지하는데 앱이
읽을 방법이 없어 통째로 유실됐다 — 앱에 `Read Device Result`를 추가해 복구 경로를 만들었다.
**D-3(미계수 손실 경로)은 여전히 미해결**이며, 다음 라운드에서 절전 시나리오만 단독
재현한 뒤 새 세션으로 카운터를 읽어 판정한다.
...
`696fbda`(펌웨어) / `06158a3`(the-via-eerraa, 이전 해시 `94fd725` — main 정리로 리베이스됨) 구현을 실기 검증 투입 가능 여부 기준으로
