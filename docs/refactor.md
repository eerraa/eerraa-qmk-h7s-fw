...
## 적용 현황 (V251122R4 시점)
- 반영됨
  - 타이머 분기 순서 최적화 및 모드별 이펙트/주기 캐시: 만료 체크를 선행하고 캐시가 깨질 때만 PROGMEM 접근을 수행해 250 kHz 호출 경로의 연산 낭비를 제거.
  - 비활성 상태 조기 반환: rgblight disable + 긴급 플래그/Velocikey 모두 없음이면 `rgblight_task()`를 즉시 종료.
  - 전면 인디케이터 처리: 전면 인디케이터 덮어쓰기 중에도 렌더 큐잉을 유지하도록 복원해 오버레이 누락을 방지.
  - 타이머 시각 공유: `rgblight_task()`에서 읽은 시각을 `rgblight_timer_task()`에 전달해 중복 타이머 읽기를 제거(긴급 경로 포함).
- 미반영/후순위
  - 호출 시각 공유: 타이머 읽기 공유 및 게이트 연동은 우선도 낮음, 필요 시 소규모 패치로 추가 검토.
  - 경량 계측: 최적화 전후 지터 측정 필요 시 빌드 플래그 형태로 한시 적용 검토.

## 다음 할 일
- 장기 대기/All Off/전면 인디케이터/일반 RGB 모드별로 장시간 테스트해 프리징·색상 누락 재발 여부 확인.
- 필요시 호출 시각 공유 최적화와 계측 플래그 추가 여부 결정.
