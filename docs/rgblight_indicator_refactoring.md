...
## 추가 시나리오 점검 (range_target 캐시 제거)
1. **범위 테이블 재적용 + 인디케이터 활성**: 포트 계층이 `rgblight_indicator_set_ranges()`를 반복 호출할 때, 센티널 없이도 테이블 재적용 직후 `rgblight_indicator_apply_target_range()`가 현재 범위와 오버레이 상태를 즉시 다시 계산해 재렌더 플래그가 유지된다.【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L203-L232】【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L388-L395】
2. **비활성 상태 범위 갱신 → 호스트 토글**: 인디케이터가 꺼져 있는 동안 범위를 바꿔도 추가 캐시 필드 없이 구성 구조체와 범위만 갱신되며, 이후 호스트 LED가 켜질 때 `rgblight_indicator_commit_state()`가 재렌더를 다시 예약해 출력이 즉시 복구된다.【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L341-L360】【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L405-L408】
3. **잘못된 타깃 입력 보정**: 범위 테이블이 비워진 상태에서 유효 범위를 벗어난 타깃이 선택되더라도, 타깃 캐시 제거 이후에도 `rgblight_indicator_apply_target_range()`가 즉시 OFF 타깃으로 교체해 안전하게 비활성 처리한다.【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L203-L232】
...
- `range_target` 센티널 필드와 관련 매크로를 제거해 더 이상 사용되지 않는 상태 캐시를 없애고, 테이블 기반 범위 재적용만 유지했다.【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L132-L155】【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L388-L395】
...
- 범위 타깃 캐시를 제거하면서 `rgblight_indicator_set_ranges()`가 테이블 재적용 후 즉시 동일 경로를 재사용해, 별도 무효화 단계 없이도 상태 전이가 일관되게 유지된다.【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L203-L232】【F:src/ap/modules/qmk/quantum/rgblight/rgblight.c†L388-L395】
