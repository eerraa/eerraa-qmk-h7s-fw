...
5. **HID 폴링 통계**: `usbHidMeasureRateTime()`은 새로 추가된 `usbBootModeGetExpectedIntervalUs()`를 통해 중앙 테이블 값을 조회하고, 초과 지연·큐 길이 통계를 기존과 동일하게 계산한다. BootMode별 기대 간격 상수가 한 곳에서 관리되므로 로그와 큐 계산이 모두 일관성을 유지한다.
