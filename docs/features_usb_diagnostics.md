...
**모드 비교는 정규화 배수가 아니라 절대 µs로 하십시오.** 1차 실기(BRICK60)에서 FS 1K/HS 2K/HS 4K의 평균 지연이 231/224/232 µs로 사실상 같았는데 정규화값은 0.23×/0.45×/0.93×로 갈렸습니다. 리포트 방출이 디바운스의 1 ms 틱 경계에 묶여 있어 지연의 절대 폭(100~130 µs)이 폴링 간격과 무관하기 때문입니다. 정규화 배수는 "그 모드가 자기 간격 예산 안에 있었는가"라는 별개 질문에만 답합니다.
