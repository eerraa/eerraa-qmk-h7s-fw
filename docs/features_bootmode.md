...
- // V251108R8: `usbProcess()`는 BootMode Apply 큐와 USB 모니터 다운그레이드 큐 모두 비어 있을 때 즉시 반환해 메인 루프 부하를 줄입니다.【F:src/hw/driver/usb/usb.c†L330-L360】
...
- ## 10. VIA 연동 (Brick60)
- // V251108R2: channel 13 value ID 1(부트 모드 선택)은 런타임 폴링 레이트를 바꾸지 않고 보류 값(`pending_boot_mode`)만 갱신하며, value ID 2(Apply 토글)가 1이 될 때 `usbBootModeScheduleApply()`를 호출해 CLI `boot set`과 동일한 EEPROM 저장→리셋 흐름을 수행합니다.【F:src/ap/modules/qmk/port/usb_bootmode_via.c†L5-L87】
...
- // V251108R8: `usbBootModeScheduleApply()`는 동일 모드에 대한 중복 요청이 큐에 이미 존재하면 무시해 불필요한 리셋 예약을 막습니다.【F:src/hw/driver/usb/usb.c†L194-L208】
...
- // V251109R6: BootMode/EEPROM 등 VIA 리셋 경로는 `resetToReset()` 전에 `USBD_Stop()`/`USBD_DeInit()`을 호출해 호스트가 디바이스 분리를 감지하도록 강제합니다.【F:src/hw/driver/usb/usb.c†L362-L376】
