...
## 10. WS2812 DMA 비동기 서비스 지침 (V251010R1)
- WS2812 DMA 재기동은 `ws2812RequestRefresh()`로 요청하고, `keyboard_task()` 말미에서 `ws2812ServicePending()`을 호출해 메인 루프에서 일괄 처리합니다.
- `ws2812ServicePending()` 호출 위치는 하나로 유지해 루프당 서비스 횟수를 최소화하고, 다른 모듈에 중복 호출을 추가하지 않습니다.
- `_DEF_FIRMWATRE_VERSION`이 `V251010R1` 이상이면 상기 구조를 준수하도록 검토합니다.
