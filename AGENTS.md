...
## 4. 변경 이력 규칙
- 코드 변경마다 수정일자에 맞춘 `// VYYMMDDRn ...` 형식의 변경 이력 주석을 추가합니다. 동일 날짜 내 추가 수정 시 `Rn` 번호를 증가시킵니다.
- 코드 변경마다 `_DEF_FIRMWATRE_VERSION`을 최신 `VYYMMDDRn`과 반드시 동기화합니다.
- 버전 문자열 작성 시 기준 날짜와 시간은 **KST(UTC+9)**을 따릅니다.
