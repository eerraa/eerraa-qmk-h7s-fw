...
// V251010R5 VIA 명령 처리 모듈 분리 및 헬퍼 정리
...
{
  led_port_indicator_flush_config(led_type);  // V251010R5 LED 타입 가드 헬퍼 경유 저장
}
...
void via_qmk_led_command(uint8_t led_type, uint8_t *data, uint8_t length)
{
  ...
  {
    return;  // V251010R5 VIA 명령 유효성 검사: 데이터 포인터/길이 확인
  }
  ...
  uint8_t  payload_length    = length - 2;  // V251010R5 VIA 페이로드 가용 길이 계산
  ...
  switch (*command_id)
  {
    case id_custom_set_value:
      ...
      {
        *command_id = id_unhandled;  // V251010R5 VIA 서브커맨드 실패 시 에러 보고
      }
      break;
...
static bool via_qmk_led_get_value(uint8_t led_type, uint8_t *data, uint8_t length)
{
  if (data == NULL || length == 0)
  {
    return false;  // V251010R5 VIA 응답 버퍼 가용성 검증
  }
  ...
  switch (*value_id)
  {
    case id_qmk_led_enable:
      if (value_length < 1)
      {
        return false;  // V251010R5 VIA 응답 길이 부족 시 실패 처리
      }
...
static bool via_qmk_led_set_value(uint8_t led_type, uint8_t *data, uint8_t length)
{
  if (data == NULL || length == 0)
  {
    return false;  // V251010R5 VIA 설정 페이로드 가용성 검증
  }
  ...
  uint8_t *value_data   = &(data[1]);
  uint8_t  value_length = (length > 0) ? (length - 1) : 0;
  bool     needs_refresh = false;  // V251010R5 설정 변경 시에만 RGBlight 갱신
  ...
  switch (*value_id)
  {
    case id_qmk_led_enable:
    {
      if (value_length < 1)
      {
        return false;  // V251010R5 VIA 설정 길이 부족 시 실패 처리
      }
    ...
    case id_qmk_led_brightness:
      ...
      if (config->hsv.v != value_data[0])
      {
        ...
        led_port_indicator_mark_color_dirty(led_type);  // V251010R5 밝기 변경 시 색상 캐시 무효화
        needs_refresh  = true;
      }
      break;
    case id_qmk_led_color:
    {
      ...
      if (config->hsv.h != hue || config->hsv.s != saturation)
      {
        ...
        led_port_indicator_mark_color_dirty(led_type);  // V251010R5 색상 변경 시 캐시 재계산
        needs_refresh  = true;
      }
      break;
    }
    default:
      return false;  // V251010R5 알 수 없는 VIA 서브커맨드 거부
  }
