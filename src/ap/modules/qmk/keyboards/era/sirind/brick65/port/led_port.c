...
EECONFIG_DEBOUNCE_HELPER(led_scroll, EECONFIG_USER_LED_SCROLL, led_config[LED_TYPE_SCROLL]); // Added
...
void led_init_ports(void)
{
  ...
  eeconfig_init_led_scroll(); // Added
...
void led_update_ports(led_t led_state)
{
  ...
  led_color = WS2812_COLOR_OFF; // Added
...
void via_qmk_led_save(uint8_t led_type)
{
  ...
  if (led_type == LED_TYPE_SCROLL) // Added
  {
    eeconfig_flush_led_scroll(true);
  }
}