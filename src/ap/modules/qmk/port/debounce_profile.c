...
typedef struct
{
  ...
  bool                      initialized;
} debounce_profile_state_t;  // V260831R2: VIA STATUS 전용 복사 상태 제거
...
bool debounce_profile_handle_via_command(uint8_t *data, uint8_t length)
{
  ...
  switch (*command_id)
  {
    ...
    case id_custom_get_value:
      handled = debounce_profile_get_value_internal(*value_id, value_data);  // V260831R2: 알 수 없는 value는 ERA처럼 id_unhandled
      break;
