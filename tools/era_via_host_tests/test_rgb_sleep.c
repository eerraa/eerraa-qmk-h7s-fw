...
int main(void)
{
  ...
  expect_true("three reasons stay lit below idle",
              !rgb_sleep_policy_local_requested(false, false, 60, 59999));
  expect_true("explicit suspend is an independent reason",
              rgb_sleep_policy_local_requested(true, false, 60, 0));
  expect_true("frame loss is an independent reason",
              rgb_sleep_policy_local_requested(false, true, 60, 0));
  expect_true("idle timeout is an independent reason",
              rgb_sleep_policy_local_requested(false, false, 60, 60000));
  expect_true("zero timeout does not idle-sleep",
              !rgb_sleep_policy_local_requested(false, false, 0, UINT32_MAX));
  expect_true("wraparound elapsed still darkens",
              rgb_sleep_policy_local_requested(false, false, 600, UINT32_MAX));
